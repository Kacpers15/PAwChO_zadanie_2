...
Dane cache są przechowywane w dedykowanym repozytorium na Docker Hubie pod tagiem powiązanym z gałęzią (`weather-app-cache:main`) z flagą `mode=max`.
        ...
        2.3. `mode=max` w przeciwieństwie do trybu domyślnego `mode=min`, zapisuje warstwy pamięci podręcznej dla wszystkich etapów pośrednich. 
              Dzięki temu przy kolejnych uruchomieniach potoku, pobrane zależności Go są błyskawicznie odtwarzane z cache, co znacząco skraca czas budowania
